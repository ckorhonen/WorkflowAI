...
type Props = {
  ...
  inProgressIndexes: number[];
  streamedChunks: (StreamedChunk | undefined)[];
  performRuns: (indexes?: number[]) => Promise<void>;
  stopRun: (index: number) => void;
  input: GeneralizedTaskInput | undefined;
};

function useTaskRunner(index: 0 | 1 | 2, props: Props) {
  const { runs, inProgressIndexes, streamedChunks, performRuns, stopRun, input } = props;

  const inputStatus = useInputMatchStatus(runs[index]?.task_input, input);

  const execute = useCallback(async () => {
    await performRuns([index]);
  }, [performRuns, index]);
  ...
  const loading = inProgressIndexes.includes(index);
  const streamedChunk = streamedChunks[index];
  const data = runs[index];
...
export function useProxyRunners(props: Props) {
