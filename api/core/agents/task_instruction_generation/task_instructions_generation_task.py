...
When the user has passed there existing instructions in 'chat_messages', you MUST generate instructions that are identical to the initial user prompt. The include eventual conditional statements from Jinja2.

# Instruction Templating specifics.
ALWAYS format the instructions as a Jinja2 template and follow the following guidelines:
{INSTRUCTION_TEMPLATING_GUIDELINES}

Ensure ALL variables from 'agent.input_json_schema' are properly injected using Jinja2 syntax.
"""
