...
class MCPService:
    ...
    async def get_ai_engineer_response(
        ...
        agent_model_parameter: str | None,
        message: str,
    ) -> MCPToolReturn:
        """Get a response from WorkflowAI's AI engineer (meta agent)."""
        try:
            if not agent_model_parameter or "/" not in agent_model_parameter or agent_model_parameter == "new":
                # Case of 'an open question' without agent ID
                last_messages: list[MetaAgentChatMessage] = []
                async for messages in self.meta_agent_service.stream_ai_engineer_without_agent_id_response(
                    messages=[MetaAgentChatMessage(role="USER", content=message)],
                ):
            ...
            return MCPToolReturn(
                success=False,
                error=f"Agent {agent_model_parameter} not found",
            )
        except Exception as e:
            return MCPToolReturn(
                success=False,
                error=f"Failed to get AI engineer response: {str(e)}",
            )
