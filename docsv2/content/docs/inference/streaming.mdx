...
<Callout type='success'>
  Streaming is available exactly like in the OpenAI API. If you are already streaming from the OpenAI API, no code
  change is required.
</Callout>
...
By setting the `stream_options.aggregate_content` parameter to `true`, WorkflowAI can aggregate the deltas into valid partial JSON objects, transforming the above deltas into:
...
streamer = await openai_client.chat.completions.create(
    ...
    messages=...,
    stream=True,
    # The following can break typing since `aggregate_content` is not supported by OpenAI,
    # one solution is to cast to ChatCompletionStreamOptionsParam
    stream_options={"aggregate_content": True},
